...
type AppState =
  ...
  | { readonly kind: "ready"; readonly bridge: ReturnType<typeof createLiveBridge> }
...
const ErrorScreen = ({ path, message, colors }: { readonly path: string; readonly message: string; readonly colors: ColorPalette }) => (
  <box flexGrow={1} justifyContent="center" alignItems="center" backgroundColor={colors.background}>
    <box width={72} flexDirection="column" padding={2} backgroundColor={colors.panel}>
      ...
      <text fg={colors.muted}>Fix the explicit SPOTIFY_CONFIG file and restart.</text>
    </box>
  </box>
)

const statusText = (state: { readonly status: string; readonly error: string | null }) => {
...
const DetailPanel = ({ snapshot, colors }: { readonly snapshot: BridgeSnapshot; readonly colors: ColorPalette }) => {
  const items = snapshot.playlistItems?.items ?? snapshot.collections[snapshot.activeCollection].items
  const selected = items[snapshot.selectedIndex] as { readonly kind?: string; readonly name?: string; readonly id?: string | null } | undefined
  return (
    <box width={30} flexDirection="column" padding={1} borderStyle="single" borderColor={colors.separator}>
...
const CommandPalette = ({ index, colors }: { readonly index: number; readonly colors: ColorPalette }) => {
  return (
    <box position="absolute" left={12} top={3} width={56} flexDirection="column" padding={1} backgroundColor={colors.panel} borderStyle="single" borderColor={colors.accent}>
      ...
      {commandDefinitions.map((command, commandIndex) => (
        <box key={command.id} backgroundColor={commandIndex === index ? colors.selectedBackground : undefined}>
          <text fg={commandIndex === index ? colors.selectedText : colors.text}>{commandIndex === index ? "> " : "  "}{command.title}</text>
        </box>
      ))}
  </box>
  )
...
const ReadyScreen = ({ bridge, snapshot, colors }: { readonly bridge: ReturnType<typeof createLiveBridge>; readonly snapshot: BridgeSnapshot; readonly colors: ColorPalette }) => {
  const [paletteOpen, setPaletteOpen] = useState(false)
  const [paletteIndex, setPaletteIndex] = useState(0)
  const items = snapshot.playlistItems?.items ?? snapshot.collections[snapshot.activeCollection].items
  const currentState = snapshot.playlistItems ?? snapshot.collections[snapshot.activeCollection]
  ...
  useKeyboard((event) => {
    ...
    if (paletteOpen) {
      ...
      if (key === "up" || key === "k") return setPaletteIndex((current) => (current + commandDefinitions.length - 1) % commandDefinitions.length)
      if (key === "down" || key === "j") return setPaletteIndex((current) => (current + 1) % commandDefinitions.length)
      if (key === "return" || key === "enter") {
    ...
    if (numeric >= 1 && numeric <= collectionKeys.length) runCommand(commandDefinitions[numeric - 1]!.id)
  })
    ...
    <box flexGrow={1} flexDirection="column" backgroundColor={colors.background}>
      ...
      <box height={1} flexDirection="row" backgroundColor={colors.panel}>
        {collectionKeys.map((collection, index) => (
          <text key={collection} fg={collection === snapshot.activeCollection ? colors.accent : colors.muted}>
            {index + 1} {collectionLabels[collection]}
          </text>
        ))}
      </box>
      <box flexGrow={1} flexDirection="row">
        <box width={24} flexDirection="column" padding={1} borderStyle="single" borderColor={colors.separator}>
          <text fg={colors.accent}>LIBRARY</text>
          {collectionKeys.map((collection, index) => (
            <box key={collection} backgroundColor={collection === snapshot.activeCollection ? colors.selectedBackground : undefined}>
              <text fg={collection === snapshot.activeCollection ? colors.selectedText : colors.text}>{collection === snapshot.activeCollection ? ">" : " "} {index + 1} {collectionLabels[collection]}</text>
            </box>
          ))}
          <box flexGrow={1} />
        ...
        <box flexGrow={1} flexDirection="column" padding={1}>
          <text fg={colors.accent}>{snapshot.playlistId ? "PLAYLIST ITEMS" : collectionLabels[snapshot.activeCollection].toUpperCase()}</text>
          <text fg={colors.muted}>{statusText(currentState)}</text>
          {items.length === 0 && currentState.status !== "loading" ? <text fg={colors.muted}>No items loaded.</text> : null}
          {items.map((item, index) => {
            const named = item as { readonly name?: string; readonly kind?: string; readonly key?: string }
            const selected = index === snapshot.selectedIndex
            return (
              <box key={named.key ?? `${named.kind}-${index}`} backgroundColor={selected ? colors.selectedBackground : undefined}>
                <text fg={selected ? colors.selectedText : colors.text}>{selected ? ">" : " "} {String(index + 1).padStart(2, "0")} {named.name ?? named.kind ?? "Unavailable"}</text>
              </box>
            )
          })}
          <box flexGrow={1} />
          <text fg={colors.muted}>j/k move  Enter open  n load more  r refresh  b back</text>
        </box>
        <DetailPanel snapshot={snapshot} colors={colors} />
      </box>
      {paletteOpen ? <CommandPalette index={paletteIndex} colors={colors} /> : null}
    </box>
  )
}
...
const App = () => {
  ...
  useEffect(() => {
    if (configPath.kind === "invalid") {
      setState({ kind: "error", path: configPath.path, message: configPath.message })
      return
    }
    void Effect.runPromise(readConfig(configPath.path)).then(
      (result) => {
        if (result.kind === "missing") {
          setState({ kind: "login", path: result.path, message: "No credentials are configured yet." })
      ...
      (error) => {
        setState({ kind: configPath.kind === "override" ? "error" : "login", path: configPath.path, message: String(error) })
      }
    )
  }, [])
