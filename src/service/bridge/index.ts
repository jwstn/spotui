...
import type { SpotifyCredentials } from "#/service/config"
import type { BearerToken } from "#/service/auth/types"
import {
  Continuation,
  LibraryCollection,
  Track,
...
import type { SpotifyApiShape } from "#/service/spotifyApi"
import type { PlaybackEvent, PlaybackHandle, PlaybackHost } from "#/player"
import { CollectionStatus } from "#/service/bridge/types"

export const CollectionState = <Item extends Schema.Constraint>(item: Item) =>
  Schema.Struct({
    status: CollectionStatus,
    items: Schema.Array(item),
    continuation: Schema.NullOr(Continuation),
    error: Schema.NullOr(Schema.String),
    generation: Schema.Number,
    loadingMore: Schema.Boolean,
  })

export const TrackCollectionState = CollectionState(Track)
export type TrackCollectionState = typeof TrackCollectionState.Type

export const initialCollectionState = <
  T extends Schema.Constraint,
>(): typeof CollectionState<T> => ({
  status: "idle",
  items: [],
  continuation: null,
  error: null,
...
export const PlaybackStatus = Schema.Literals([
  "idle",
  "loading",
  "playing",
  "pasued",
...
export const PlaybackState = Schema.Struct({
  ...
  track: Schema.NullOr(NowPlaying),
  error: Schema.NullOr(Schema.String),
  output: Schema.Literals(["local", "none"]),
})
export type PlaybackState = typeof PlaybackState.Type

export const BridgeSnapshot = Schema.Struct({
  auth: Schema.Literals(["loading", "login", "ready", "error"]),
  authError: Schema.NullOr(Schema.String),
  activeCollection: LibraryCollection,
  selectedIndex: Schema.Number,
  playlistId: Schema.NullOr(Schema.String),
  playlistItems: Schema.NullOr(TrackCollectionState),
})
export type BridgeSnapshot = typeof BridgeSnapshot.Type
